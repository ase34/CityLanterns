...
public class LanternSelectListener implements Listener {
    ...
    public void onPlayerInteractBlock(PlayerInteractEvent ev) {
        ...
        if (ev.getClickedBlock().getType() != Material.REDSTONE_LAMP_OFF
                && ev.getClickedBlock().getType() != Material.REDSTONE_LAMP_ON) {
        ...
        if (ev.getAction() == Action.LEFT_CLICK_BLOCK) {
            if (selected != null && selected.getGroup().equals(group)) {
                ...
                ev.getPlayer().sendMessage(
                    ChatColor.GOLD + "Removed this lantern from group " + ChatColor.GRAY + selected.getGroup()
                            + ChatColor.GOLD + "!");
            } else {
                if (selected == null) {
                ...
                ev.getPlayer().sendMessage(
                    ChatColor.GOLD + "Added this lantern to group " + ChatColor.GRAY + group + ChatColor.GOLD + "!");
            }
        } else {
            if (selected == null) {
                ...
                ev.getPlayer().sendMessage(
                    ChatColor.GOLD + "This lantern currently is in group " + ChatColor.GRAY + selected.getGroup()
                            + ChatColor.GOLD + "!");
            }
        }
        ev.setCancelled(true);
